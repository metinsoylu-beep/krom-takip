...
assert.equal(invoiceValues[1][0], "65 ₺", "Açık borç yalnızca pozitif cari bakiyeleri toplamalı");
assert.equal(invoiceValues[1][1], "15 ₺", "Açık alacak yalnızca negatif cari bakiyeleri toplamalı");
assert.equal(invoiceValues[1][2], "50 ₺", "Net borç açık borç ile açık alacak farkı olmalı");
...
const manuelYedek = post({ action:"backups.create", aciklama:"  Ay   sonu kapanışı  " });
...
assert.equal(manuelYedekDetayi.backup.aciklama, "Manuel yedek · Ay sonu kapanışı", "Manuel yedek adı temizlenerek saklanmalı");
