...
assert.match(html, /grid-template-columns: max-content max-content minmax\(0, 1fr\)/, "Masaüstünde başlık ve kur grupları sola yaslanmalı");
assert.match(html, /\.kur-satirlar \{[\s\S]*?justify-content: start;/, "USD ve EUR grubu soldan başlamalı");
assert.match(html, /\.kur-islemler \{[\s\S]*?justify-content: flex-end;/, "Kur işlem düğmeleri sağda kalmalı");
